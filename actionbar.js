...
function load_action_images(){
    im.tmp.act = loadImage('images/act.png');
    im.tmp.attack = loadImage('images/attack.png');
    im.tmp.item = loadImage('images/item.png');
    im.tmp.spare = loadImage('images/spare.png');
    im.tmp.super = loadImage('images/super.png');
}
...
function process_images(){
    for (const [img_name, img] of Object.entries(im.tmp)) {
        ...
        im[img_name] = {
            normal: create_hl(img, color(21, 164, 189)),
            hl: create_hl(img, color(86, 240, 43))
        };
    }
}
